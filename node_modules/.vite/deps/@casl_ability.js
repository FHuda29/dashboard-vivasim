...
function P2(t3, i4) {
  if (i4) {
    if (!C(i4, R2)) Object.defineProperty(i4, R2, { value: t3 });
    else if (t3 !== i4[R2]) throw new Error(`Trying to cast object to subject type ${t3} but previously it was casted to ${i4[R2]}`);
  }
  return i4;
}
var T = (t3) => {
  const i4 = typeof t3;
  return "string" === i4 || "function" === i4;
};
var B2 = (t3) => t3.modelName || t3.name;
var S2 = (t3) => "string" === typeof t3 ? t3 : B2(t3);
function q3(t3) {
  if (C(t3, R2)) return t3[R2];
  return B2(t3.constructor);
}
function z3(t3, i4, e3) {
  let s3 = F(i4);
  let n3 = 0;
  while (n3 < s3.length) {
    const i5 = s3[n3++];
    if (C(t3, i5)) s3 = e3(s3, t3[i5]);
  }
  return s3;
}
function D(t3, i4) {
  if ("string" === typeof i4 && -1 !== t3.indexOf(i4)) return i4;
  for (let e3 = 0; e3 < i4.length; e3++) if (-1 !== t3.indexOf(i4[e3])) return i4[e3];
  return null;
}
var Y = (t3, i4) => t3.concat(i4);
function k(t3, i4) {
  if (i4 in t3) throw new Error(`Cannot use "${i4}" as an alias because it's reserved action.`);
  const e3 = Object.keys(t3);
  const s3 = (t4, e4) => {
    const s4 = D(t4, e4);
    if (s4) throw new Error(`Detected cycle ${s4} -> ${t4.join(", ")}`);
    const n3 = "string" === typeof e4 && e4 === i4 || -1 !== t4.indexOf(i4) || Array.isArray(e4) && -1 !== e4.indexOf(i4);
    if (n3) throw new Error(`Cannot make an alias to "${i4}" because this is reserved action`);
    return t4.concat(e4);
  };
  for (let i5 = 0; i5 < e3.length; i5++) z3(t3, e3[i5], s3);
}
function I(t3, i4) {
  if (!i4 || false !== i4.skipValidate) k(t3, i4 && i4.anyAction || "manage");
  return (i5) => z3(t3, i5, Y);
}
function L(t3, i4, e3) {
...
var K = class {
  constructor(t3, i4, e3 = 0) {
    ...
    this.fields = t3.fields ? F(t3.fields) : void 0;
    this.priority = e3;
    this.t = i4;
  }
  i() {
  ...
  matchesConditions(t3) {
    ...
    if (!t3 || T(t3)) return !this.inverted;
    const i4 = this.i();
    return i4(t3);
  }
  matchesField(t3) {
...
var Z = (t3, i4) => {
  if (!t3.h && i4.fields) t3.h = true;
};
var tt = class {
  constructor(t3 = [], i4 = {}) {
    this.h = false;
    this.l = /* @__PURE__ */ new Map();
    this.p = { conditionsMatcher: i4.conditionsMatcher, fieldMatcher: i4.fieldMatcher, resolveAction: i4.resolveAction || H };
    this.g = i4.anyAction || "manage";
    this.$ = i4.anySubjectType || "all";
    this.A = i4.detectSubjectType || q3;
    this.M = t3;
    this.j = this.m(t3);
  }
  get rules() {
  ...
  m(t3) {
    ...
    for (let e3 = t3.length - 1; e3 >= 0; e3--) {
      const s3 = t3.length - e3 - 1;
      const n3 = new K(t3[e3], this.p, s3);
      const r2 = F(n3.action);
      const o3 = F(n3.subject || this.$);
      Z(this, n3);
      for (let t4 = 0; t4 < o3.length; t4++) {
        const e4 = G(i4, o3[t4], X);
        for (let t5 = 0; t5 < r2.length; t5++) G(e4, r2[t5], W).rules.push(n3);
      }
    }
    return i4;
  }
  possibleRulesFor(t3, i4 = this.$) {
    if (!T(i4)) throw new Error('"possibleRulesFor" accepts only subject types (i.e., string or class) as the 2nd parameter');
    const e3 = G(this.j, i4, X);
    const s3 = G(e3, t3, W);
    if (s3.merged) return s3.rules;
    const n3 = t3 !== this.g && e3.has(this.g) ? e3.get(this.g).rules : void 0;
    let r2 = U(s3.rules, n3);
    if (i4 !== this.$) r2 = U(r2, this.possibleRulesFor(t3, this.$));
    s3.rules = r2;
    s3.merged = true;
    return r2;
  }
  rulesFor(t3, i4, e3) {
    ...
    if (e3 && "string" !== typeof e3) throw new Error("The 3rd, `field` parameter is expected to be a string. See https://stalniy.github.io/casl/en/api/casl-ability#can-of-pure-ability for details");
    if (!this.h) return s3;
    return s3.filter((t4) => t4.matchesField(e3));
  }
  on(t3, i4) {
    const e3 = this.l.get(t3) || null;
    const s3 = N3(i4, e3);
    this.l.set(t3, s3);
    return () => {
      const i5 = this.l.get(t3);
      if (!s3.next && !s3.prev && i5 === s3) this.l.delete(t3);
      else if (s3 === i5) this.l.set(t3, s3.prev);
      Q(s3);
    };
  }
  v(t3, i4) {
    let e3 = this.l.get(t3) || null;
    while (null !== e3) {
...
var st = (i4, e3, s3) => p4(Object.assign({}, it, i4), Object.assign({}, et, e3), s3);
var nt = p4(it, et);
var rt = /[-/\\^$+?.()|[\]{}]/g;
var ot = /\.?\*+\.?/g;
var ct = /\*+/;
...
function ht(t3, i4, e3) {
  ...
  const r2 = t3.replace(ut, "\\$&").replace(ct, n3 + s3);
  return i4 + t3.length === e3.length ? `(?:${r2})?` : r2;
}
function lt(t3, i4, e3) {
  if ("." === t3 && ("*" === e3[i4 - 1] || "*" === e3[i4 + 1])) return t3;
  return `\\${t3}`;
}
function at(t3) {
  const i4 = t3.map((t4) => t4.replace(rt, lt).replace(ot, ht));
...
var ft = (t3) => {
  let i4;
  return (e3) => {
    if ("undefined" === typeof i4) i4 = t3.every((t4) => -1 === t4.indexOf("*")) ? null : at(t3);
    return null === i4 ? -1 !== t3.indexOf(e3) : i4.test(e3);
  };
};
var Ability = class extends PureAbility {
  constructor(t3 = [], i4 = {}) {
    super(t3, Object.assign({ conditionsMatcher: nt, fieldMatcher: ft }, i4));
  }
};
function createMongoAbility(t3 = [], i4 = {}) {
  return new PureAbility(t3, Object.assign({ conditionsMatcher: nt, fieldMatcher: ft }, i4));
}
...
var AbilityBuilder = class {
  ...
  can(t3, i4, e3, s3) {
    ...
    if (i4) {
      n3.subject = i4;
      if (Array.isArray(e3) || "string" === typeof e3) n3.fields = e3;
      else if ("undefined" !== typeof e3) n3.conditions = e3;
      if ("undefined" !== typeof s3) n3.conditions = s3;
    }
    this.rules.push(n3);
    return new dt(n3);
  }
  cannot(t3, i4, e3, s3) {
    const n3 = this.can(t3, i4, e3, s3);
    n3.O.inverted = true;
    return n3;
  }
  build(t3) {
    return isAbilityClass(this._) ? new this._(this.rules, t3) : this._(this.rules, t3);
  }
};
function defineAbility(t3, i4) {
  const e3 = new AbilityBuilder(createMongoAbility);
  const s3 = t3(e3.can, e3.cannot);
  if (s3 && "function" === typeof s3.then) return s3.then(() => e3.build(i4));
  return e3.build(i4);
}
var bt = (t3) => `Cannot execute "${t3.action}" on "${t3.subjectType}"`;
...
var ForbiddenError = class extends yt {
  ...
  unlessCan(t3, i4, e3) {
    ...
    this.action = t3;
    this.subject = i4;
    this.subjectType = S2(this.ability.detectSubjectType(i4));
    this.field = e3;
    const n3 = s3 ? s3.reason : "";
    this.message = this.message || n3 || this.constructor.F(this);
    return this;
  }
};
ForbiddenError.F = bt;
...
export {
  ...
  ForbiddenError,
  PureAbility,
  st as buildMongoQueryMatcher,
  I as createAliasResolver,
  createMongoAbility,
  defineAbility,
  q3 as detectSubjectType,
  ft as fieldPatternMatcher,
  bt as getDefaultErrorMessage,
  pt as hkt,
  nt as mongoQueryMatcher,
  P2 as subject,
  F as wrapArray
};
